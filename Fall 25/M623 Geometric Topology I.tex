...
\begin{document}
    ...
    \begin{definition}
        ...
        \begin{enumerate}[label=\arabic*)]
            \item \(\forall\) vector bundle \(\xi\), assign \(w_i(\xi) \in H^i(B(\xi);\mathbb{F}_2)\) so that \(w_0(\xi) = 1\) and \(w_i(\xi) = 0\) for \(i > n\) when \(\xi\) is a an \(n\)-plane bundle.
            \item \textit{Naturality}: For continuous \(f: B \to B(\xi)\), we have \(w_i(f^{\ast} \xi) = f^{\ast} (w_i \xi) \in H^i(B;\mathbb{F}_2)\). [First one is the pullback on the bundle, second one is the induced map on the cohomology.]
            \item \textit{Whitney Sum Formula}: If \(\xi , \eta\) are vector bundles over \(B\) we have: \(w_k(\xi \oplus \eta) = \sum_{i+j=k} w_i(\xi) \cup w_j(\eta) \).
            \item \(0 \neq w_1(\gamma^1_1) \in H^1(P^1;\mathbb{F}_2) = H^1(S^1; \mathbb{F}_2) = \mathbb{F}_2\).
        \end{enumerate} 
    ...
    then \(\eta = f^{\ast} \xi, w_i(\eta) = f^{\ast} w_i(\xi)\).

    Note: axioms 1 and 2 says \(w_i\) are \textit{characteristic classes}. Characteristic Classes are cohomology classes respecting naturality. Meaning they respect nontriviality of bundles. Just like homology `classifies' upto homotopy in a sense, we need characteristic classes to capture the `twists' in a vector bundle.

    Axiom 1 and 2 implies:

    \begin{proposition}
        [1] \(\xi \cong \eta \implies w_i(\xi) = w_i(\eta)\).
    \end{proposition}

    Recall that vector bundlles are isomorphic if:

    \[
        \begin{tikzcd}
            E(\xi) \ar[rr,"\cong"] \ar[rd] & & E(\eta) \\ & B
        \end{tikzcd}
    \]
    ...
    \begin{proof}
        ...
        \(w_i(\epsilon^n_B) = w_i (c^{\ast} \epsilon^n_{\text{pt}}) = c^{\ast} w_i(\epsilon^n_{\text{pt}}) \in H^i(\text{pt};\mathbb{F}_2) = 0\).
    ...
    \begin{proposition}
        [3] If \(\epsilon\) trivial then \(w_i(\epsilon \oplus \eta) = w_i(\eta)\). In other words, \(w_i\) stable characteristic classes.
    \end{proposition}
    ...
    \begin{proposition}
        ...
        \[
            w_{n-k+1}(\xi) = \cdots = w_{n-1} (\xi) = w_n(\xi) = 0
        \]
    \end{proposition}
    ...
    eg for \(n\) odd there exists a nowhere zero section of the tangent bundle \(TS^n\). Therefore, \(w_n(TS^n) = 0\).
    ...
    If \(M^n\) is a closed \(n\)-manifold then \(w_n(TM^n) \equiv \xi(M) \pmod 2\).
    ...
    \begin{proof}
        ...
        \(w_i(\xi) = w_i(\epsilon^{k \perp}_B)\) by proposition 3. Note that \(\epsilon^{k \perp}_B\) is a \(n - k\) bundle, axiom 1 implies the statement.
