...
\begin{document}
    ...
    \section*{Friday, 12/5/2025}
    
    Consider continuous \(f: X \to Y\). 
    
    The following are always true: \(f ^{-1} (\text{open}) = \text{open}, f ^{-1} (\text{closed}) = \text{closed}, f(\text{compact}) = \text{compact}\).
    
    But maybe we want \(f(\text{open}) = \text{open}, f (\text{closed}) = \text{closed}, f ^{-1} (\text{compact}) = \text{compact}\).
    
    \begin{definition}
        \(f\) proper means \(f ^{-1} (\text{compact}) = \text{compact}\). 
    \end{definition}

    \begin{theorem}
        [?]

        If \(X\) is Hausdorff then \(f: X \to Y\) is proper if and only if it is \textit{universally closed}.

        A map \(X \xrightarrow{f} Y\) is universally closed if and only if for all continuous maps \(W \to Y\) we have:

        \[
            \begin{tikzcd}
                W \times_Y X \ar[d] \ar[r,"f_W"] & W \ar[d] \\
                X \ar[r,"f"] & Y
            \end{tikzcd}
        \]
    ...
    Example: Consider \(\mathbb{Z} \to \{ \cdot \}\). This is a closed map. But it is not univerally closed: let \(W\) be a nice space with a discrete sequence which converges: \(x_1 , x_2 , x_3, \cdots \to x_0\). Then \(W \times_Y X\) fais to be closed since the associated fibers to the sequence doesn't converge in \(W \times_X Y\).

    \begin{definition}
        \(X \xrightarrow{\pi} Y\) is \textit{proper} if and only if it is separated, of finite type, and universally closed.
    \end{definition}

    \begin{theorem}
        The property of being proper is reasonable.
    \end{theorem}

    \[
        \begin{tikzcd}
            X \times_Y (Y \times_Z W) \ar[d,phantom,sloped,"="] \\
            W \times_Z X \ar[r,"cl."] \ar[d] & Y \times_Z W \ar[r,"cl."] \ar[d] & W \ar[d] \\
            X \ar[r,"u.c."] & Y \ar[r,"u.c."] & Z
        \end{tikzcd}
    \]

    \[
        \begin{tikzcd}[column sep = small]
            & & X \ar[d] \ar[r,phantom,"\supset"] & X \times_Y U_\alpha & (X \times_Y U_\alpha) \times_{U_\alpha} W \ar[l] \ar[r,"=",phantom] \ar[d] & X \times_Y W \\
            \bigcup W_\alpha \ar[r,phantom,"="] & W \ar[r]  & Y \ar[r,phantom,"\supset"] & U_\alpha & W \ar[l] 
        \end{tikzcd}
    \]

    This proves the theorem.

    Example: \(\mathbb{A}^1_k \to \operatorname{Spec} k\) is not proper.

    \[
        \begin{tikzcd}
            \mathbb{A}^1_k \ar[r,"\delta"] & \mathbb{A}^1_k \times \mathbb{P}^1_k \ar[r] & \mathbb{P}^1_k \\
            x & (x,x)
        \end{tikzcd}
    \]

    \(\operatorname{Spec} L \to \operatorname{Spec} K\) where \(K \subset L\) is proper.
    ...
    \begin{proof}
        \begin{enumerate}[label=\arabic*)]
            \item Finite \(\implies\) Affine \(\implies\) Separated.
            \item Module-finite \(\implies\) Finitely generated \(\implies\) Finite \(\implies\) Finite type
            \item Finiteness is a reasonable property. Enough to check finite morphisms are closed. 
        \end{enumerate} 
    \end{proof}    

    We also have Finite \(\implies\) Integral \(\implies\) Closed.

    Recall lying over theorem: if \(B \subset A\) is an integral extension then \(\operatorname{Spec} A \to \operatorname{Spec} B\) is surjective.

    We can reduce to proving that if \(A\) is an integral extension of \(B\) then \(\operatorname{Spec} A \xrightarrow{\pi} \operatorname{Spec} B\) is closed.

    Essentially wts \(\pi(V(I)) = V(J)\) for some \(J\).

    We have \(\pi^\sharp B \to A\) and \(I \subset A\).

    Define \(J = (\pi^\sharp) ^{-1} (I)\). We have the diagram:
    ...
    \[
        \begin{tikzcd}
            \operatorname{Spec} B & \operatorname{Spec} A \ar[l] \\
            V(J) \ar[u] & V(I) = \operatorname{Spec} A / I \ar[l]
        \end{tikzcd}
    \]

    \[
        \operatorname{Spec} A / I \to \operatorname{Spec} B / J
    \]

    \(A\) int. over \(B \implies A / I\) is an integral extension of \(B / J\). Surjectivity follows.

    \begin{theorem}
        \(\mathbb{P}_A^n \to \operatorname{Spec} A\) is proper.
    \end{theorem}

    \begin{proof}
        \begin{enumerate}[label=\arabic*)]
            \item Separated we proved last time.
            \item Finiteness is clear from the cover by finitely many copies of \(\mathbb{A}^n_A\). 
            \item Consider any \(W \to \operatorname{Spec} A\). WTS: \(\mathbb{P}^n_A \times_{\operatorname{Spec} A} W \to W\) is closed. It suffices to take an open cover of \(W\) and show that all pullbacks are closed.
        \end{enumerate} 
    \end{proof}

    \(W = \bigcup_{\alpha} \operatorname{Spec} B_\alpha\).

    \(\mathbb{P}^n_A \times_{\operatorname{Spec} A} \operatorname{Spec} B_\alpha \cong \mathbb{P}^n_{B_\alpha}\).

    \(\mathbb{P}^n_{B_\alpha} \xrightarrow{\text{closed?}} \operatorname{Spec} B_\alpha\)
    
    Closedness follows from fundamental theorem of elimination theory.

    \begin{corollary}
        Every projective variety over \(k\) is proper over \(\operatorname{Spec} k\).
    \end{corollary}

    \[
        X \to \mathbb{P}_k^n \to \operatorname{Spec} k
    \]

    \begin{theorem}
        If \(X\) is proper  over \(\operatorname{Spec} k\) and \(k = \overline{k}\) and \(X\) is integral then \(\Gamma(X,\mathcal{O}_X) = k\).
    \end{theorem}

    Non-examples: \(\operatorname{Spec} \mathbb{C} \to \operatorname{Spec} \mathbb{R}\).

    \(X = \operatorname{Spec} k \coprod \operatorname{Spec} k = \operatorname{Spec} k \times k \to \operatorname{Spec} k\)
    
    \(X = \operatorname{Spec} k[\epsilon] / (\epsilon^2) \to \operatorname{Spec} k\)

    If \(f\in \Gamma(X,\mathcal{O}_X)\) and \(f: X \to \mathbb{A}^1_k\) 

    \[
        \begin{tikzcd}
            X \ar[r] \ar[rd] & \mathbb{A}^1_k \ar[r,hook] & \mathbb{P}^1_k \ar[ld]  & \text{is proper} \\
            & \operatorname{Spec} k
        \end{tikzcd}
    \]

    The image of \(X\) in \(\mathbb{P}^1_k\) is closed. But \(X\) cannot contain the point at infinity. Thus image of \(X\) must be finite.

    The image of \(X\) in \(\mathbb{P}^1_k\) is closed.
    ...
    \begin{definition}
        ...
        The Krull dimension of a ring \(A\) is the max. length of a strictly increasing chain of prime ideals.

    \end{definition}

    \[
        P_0 \subsetneq P_1 \subsetneq \cdots \subsetneq P_n \text{ has length } n
    \]

    A maximal chain is a chain that cannot be lengthened by insertion. Note that it isn't necessarily the longest chain.

    Example:

    The krull dimension of \(k[x]\) is \(1\) since all ideals are of the form \((P(x))\) or \((c)\), and if the ideal is prime then \(P\) is irreducible or \((0)\).
    ...
    \begin{definition}
        The dimension of a scheme is the max length of a chain of `points' \(x_0 , x_1 , \cdots , x_n \in X\) such that \(x_{k+1} \in \overline{\{ x_k \}}\).
    \end{definition}

    \section*{Monday, 12/8/2025}
    
    Recall dimension. We can think of it very geometrically:

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.8\textwidth]{img/dimension}
    \end{figure}

    As in \((x_1 , \cdots , x_n) \supset (x_1 , \cdots , x_{n-1}) \supset \cdots \supset (x_1) \supset (0)\) 

    \begin{theorem}
        Suppose \(B \hookrightarrow A\) is an integral extension. Then \(\dim B = \dim A\).
    \end{theorem}

    \begin{proof}
        \(\operatorname{Spec} A \to \operatorname{Spec} B\) so if we have \(Q_0 \subsetneq Q_1 \subsetneq \cdots \subsetneq Q_n\) and \(P_0 \subsetneq P_1 \subsetneq \cdots \subsetneq P_n\) where \(P_i = Q_i \cap B\).

        Suppose \(Q_0 \subsetneq Q_1 \subsetneq \cdots \subsetneq Q_n \subset A\).

        We can define \(P_i = Q_i \cap B\).

        Then \(P_0 \subset P_1 \subset \cdots \subset P_n \subset B\).
        ...
        \[
            \begin{tikzcd}
                Q_i \ar[rd] \ar[r,"\subsetneq",phantom] & Q_{i+1} \ar[d] \ar[r,"\in",phantom] & \operatorname{Spec} A \ar[d] \\
                x_i \ar[r,"=",phantom] & P_i \ar[r,"\in",phantom] & \operatorname{Spec} B
            \end{tikzcd}
        \]

        We know that if \(B \xrightarrow{\pi^\sharp} A\) is any integral map then the fibers of \(\operatorname{Spec} A \xrightarrow{\pi} \operatorname{Spec} B\) are discrete sets. Replace \(B\) by \((B \setminus P) ^{-1} B = B_P\) and \(A\) by \((A \setminus \pi^\sharp(P)) ^{-1} A\). So we may assume \(P\) is the unique closed point of \(B\). We may replace \(B\) by \(B / P\) and \(A / P^e\) and assume \(B\) is a field.
        
        So, we claim that for an integral extension of a field, \(\operatorname{Spec} B\) is discrete.

        Suppose \(P_1 \subset P_2\) in \(A\). Replace \(A\) by \(A / P_1\). Then \(P_1\) becomes the zero ideal, \(P_2\) becomes \(P_2 / P_1\).

        Then \(B\) is a field, \(A\) is an integral domain that is integral over \(B\).

        If \(a\in A\) then it satisfies a monic polynomial over \(B\):
        ...
        \[
            \implies \frac{1}{a} = - \frac{a^{n-1} + \cdots + b_{n-1}}{b_n}
        \]

    \end{proof}

    Note, in general, \(\pi ^{-1} (y) \cong X \times_{\operatorname{Spec} \mathbb{K}(y)} Y\). So, prime ideal \(P\) of \(Y\) corresponding to \(Y\) then the associated ideal in \(A\) is \(\operatorname{Frac} (B / P) \otimes_B A\).

    \begin{definition}
        [Codimension] Suppose we have irreducible \(X \subset\) irreducible \(Y\).

        Then \(\operatorname{codim}_X Y =\) longest chain of irreducibles:

        \[
            X = X_0 \subsetneq X_1 \subsetneq X_2 \subsetneq \cdots \subsetneq X_n = Y
        \]
    \end{definition}

    Then \(\dim X + \operatorname{codim}_X Y \leq \dim  Y\).

    Now suppose \(A\) is a finitely generated \(k\)-algebra and also an integral domain.

    We can define \(K = \operatorname{Frac} (A) \supset K\).

    Then we can look at the transcendence degree. We define \(\Tr \deg_k K\).

    A subset \(S \subset K \supset k\) is algebraically independnent over \(k\) if there is no nontrivial polynomial relation with \(k\)-coefficients among elements of \(S\).

    A maximal algebraically independent set is called a transcendence basis.

    \begin{theorem}
        The cardinality of any two transcendence basis of \(K\) over \(k\) are the same.
    \end{theorem}

    To prove this we need exchange lemma.

    \begin{lemma}
        [Exchange lemma] If \(S\) is a transcendence basis and \(a\in K\) then there exists \(s\in S\) such that \((S \setminus \{ s \}) \cup \{ a \}\) is a transcendance basis. 
    \end{lemma}
    
    Then we can define the Transcendance dimension:

    \[
        \operatorname{dim} A = \text{tr deg}_k \operatorname{Frac}(A) 
    \]

    Example 1:  \(\text{tr deg}_k k(x_1 , \cdots , x_n) = n\)
    
    Example 2:\(\text{tr deg}_k \operatorname{Frac} (k [x_1 , x_2] / f(x_1 , x_2))\) if \(f\) irreducible and \(\{ x_1 \}\) and \(\{ x_2 \}\) are both tr. bases. Let \(A\) be the quotient. \(x_2\) is algebraic over \(k(x_1)\). So anything is a polynomial over \(k(x_1)\). Then anything in \(A\) is polynomial over \(k(x_1)\).

    Example 3: If \(k=\mathbb{C}\) and \(X\) is a compact Riemann surface and \(K = \operatorname{mero}(X)\). Then \(\text{tr deg}_\mathbb{C} K = 1\).
    
    \(f \in K \setminus \mathbb{C}\) must have a poly at some \(x_0 \in X\).

    \(\{ {f} \} \) must have a pole at some \(x_0\in X\). \(\{ f \} \) is algebraically independent.

    Then \(\{ f,g \} \) correspond satisfy:

    \(\{ f,g \} \) 

    \(\{ f^i g^j \} \) 

    \(\{ f^i g^j \mid0 \leq 0 < i,j \} \) 

    Big theorem:

    \begin{theorem}
        If \(A\) is an int. degree finitely generated over \(k\) then \(\text{tr deg}_k \operatorname{Frac} (A) = \dim  A\).
    \end{theorem}

    \begin{theorem}
        [Noether Normalization] If \(A\) is as above and \(n = \text{tr deg}_k \operatorname{Frac} A\) then there exists a basis \(\{ a_1 , \cdots , a_n \} \subset A\) s.t.
        
        \(A\) is integral over \(k[x_1 , \cdots , x_n]\) withthe injective map:

        \([x_1 , \cdots , x_n] \to A, x_1 \mapsto a_i\) via the injective map: \(L \)  
    \end{theorem}

    \begin{proof}
        Let \(A = k[y_1 , \cdots , y_m]\).

        \(\overline{y_1} , \cdots ., \overline{y_m} \) are generators of \(A\) so \(m \geq n\). If \(m = n\) we can just take \(a_1 =  \overline{y}_1, \cdots , a_m = \overline{y}_{m}  \) 
        
        Otherwise we find elements \(z_1 , \cdots , z_{m-1} \) of \(A\) such that \(A\) is finitely generated as a module over the \(k\)-subalgebra generated by these elements.
    \end{proof}

    \section*{Wednesday, 12/10/2025}
    
    Recall we were proving that:
    ...
    As an intermediate step we had:

    \begin{theorem}
        [Noether] Let \(A\) be a f.g. \(k\)-algebra which is also an integral domain. Let \(\text{tr deg} \operatorname{Frac} (A) = n\).

        Then \(\exists\) algebraically independent element \(a_1 , \cdots , a_m \in A\) such that \(A\) is a f.g. \(k[a_1 , \cdots , a_m]\)-module.
        
        Note that \(k[a_1 , \cdots , a_m] \cong k[x_1 , \cdots , x_m]\).
    \end{theorem}

    \begin{proof}
        We use induction on \(m\). Case that \(m = 0\) is trival: it just just the fact that a field extension which is finitely generated in the sense of rings is module-finite.

        Induction hypothesis for \(m\): if \(A\) is generated by \(b_1 , \cdots , b_m\) then it is a f.g. module over \(k[a_1 , \cdots , a_n]\) for some algebraically independent set \(\{ a_1 , \cdots , a_n \} \).

        Suppose we know this for some \(m\).

        Let \(A = k[b_1 , \cdots , b_{m+1}]\). If the \(b_i\) are algebraically independent then they are a transcendence basis for \(\operatorname{Frac} (A)\). Then \(n = m+1\). We can take \(a_1 = b_1 , a_2 = b_2, \cdots , a_{m+1} = b_{m+1}\) and we're done.

        Suppose they're not algebraically independent. Then \(\exists f(x_1 , \cdots , x_{m+1})\) a non-zero polynomial such that \(f(b_1 , \cdots , b_{m+1}) = 0\).

        Claim: There exists positive integers \(r_1 , r_2, \cdots , r_m\) such that:

        \[
            f \left( x_1 - x_{m+1}^{r_1}, x_2 - x_{m+1}^{r_2} , \cdots , x_m - x_{m+1}^{r_m}, x_{m+1} \right) 
        \]

        is a \(k\)-multiple of a monic polynomial in \(x_{m+1}\).

        To see this, note that \(f\) is a \(k[x_{m+1}]\)-linear combination of monomials in \(x_1 , \cdots, x_{m+1}\).
        
        \(x_1^{c_1} \cdots x_{m+1}^{c_{m+1}} \mapsto (c_1 , \cdots , c_{m+1})\).

        Use lexicographic order to sort the monomials.

        Consider \(x_1^{c_1} \cdots x_m^{c_m} x_{m+1}^{c_{m+1}}\).

        Plugging in,

        \[
            (x_1 - x_{m+1}^{r_1})^{c_1} (x_2 - x_{m+1}^{r_2})^{c_2} \cdots (x_m - x_{m+1}^{r_m})^{c_m} x_{m+1}^{c_{m+1}}
        \]
 
        Then the highest exponent of \(x_{m+1}\) would be \(r_1 c_1 + r_2 c_2 + \cdots + r_m c_m + c_{m+1}\).
        
        Pick \(r_1 \gg r_2 \gg r_3 \gg \cdots \gg r_m \gg 1\) so that highest lexicographic order can 'win'

        After this substitution and multiplying by an element of \(k\) we get a polynomial in \(x_1 , \cdots , x_{m+1}\) monic in \(x_{m+1}\) where:

        \(f(b_1 , \cdots , b_{m+1}) = 0\)
        
        \(g(x_1 , \cdots , x_{m+1}) = f(x_1 - x_{m+1}^{r_1} , \cdots , x_m - x_{m+1}^{r_m}, x_{m+1})\)
        
        \(0 = f(b_1 , \cdots , b_{m+1}) = g(b_1 + b_{m+1}^{r_1} , \cdots , b_m + b_{m+1}^{r_m}, b_{m+1})\)
        
        So \(b_{m+1}\) is integral over:

        \[
            k[b_1 + b_{m+1}^{r_1}, \cdots , b_m + b_{m+1}^{r_m}]
        \]

        So \(A = k[b_1 , \cdots , b_{m+1}] =\)
        
        \[
            k[b_1 + b_{m+1}^{r_1}, \cdots , b_m + b_{m+1}^{r_m}, b_m] \subset A
        \]

        Define
        
        \[
            B = k[b_1 + b_{m+1}^{r_1} , \cdots , b_m + b_{m+1}^{r_m}] \subset A
        \]

        Then \(A = B[b_{m+1}]\) and \(b_{m+1}\) is integral over \(A\).

        Thus \(A\) is a f.g. \(B\)-module.

        \(A = \operatorname{Span}_B (1, b_{m+1}, b_{m+1}^2 , \cdots , b_{m+1}^{d-1})\)

        \(d = \deg_{x_{m+1}} g\)
        
        By the induction hypothesis, \(B\) is module finite over \(k[c_1 , \cdots , c_n]\) for some ind. set \(c_1 , \cdots , c_n \in B \subset A\).
        
        \(A\) is module-finite over \(B\)

        \(B\) is module finite over \(k[c_1, \cdots , c_n]\)

        Thus \(A\) is module finite over \(k[c_1 , \cdots , c_n]\).

    \end{proof}

    We have proved, if \(n = \operatorname{tr deg Frac} A\) then there exists an injective ring homomorphism:

    \[
        k[x_1 , \cdots , x_n] \xrightarrow{\text{inj}} A  \text{ integral extension} 
    \]

    Then \(\operatorname{Spec} A \to \mathbb{A}^n\) is an integral dominant morphism.

    Then \(\dim \operatorname{Spec} A = \dim \mathbb{A}^n\) where \(n = \operatorname{tr deg Frac}(A)\).

    So we just have to prove it for affine \(n\)-space.
    ...
    \begin{proof}
        ...
        \[
            (0) \subset (x_1) \subset (x_1, x_2) \subset \cdots \subset (x_1, \cdots , x_n)
        \]

        is a strict chain of prime ideals in \(k[x_1 , \cdots , x_n]\).

        This tells us \(\dim \mathbb{A}^n_k \geq n\).

        For the other direction we use induction on \(n\).

        Suppose \(P_0 = (0) \subsetneq P_1 \subsetneq P_2 \subsetneq \cdots \subsetneq P_m\) are pprime ideals in \(k[x_1 , \cdots , x_n]\).

        Let \(f\in P_1\) be non-zero and thus non-constant. \(f\) has a non-constant irreducible factor \(g\) in \(P_1\).

        \(k[x_1 , \cdots , x_n]\) is a UFD. \(g\) generates a prime ideal. We have \((0) \subsetneq (g) \subseteq P_1\).

        Now consider \(k[x_1 , \cdots , x_n] / (g)\). It has a chain of prime ideals:

        \[
            P_1 / (g) \subsetneq P_2 / (g) \subsetneq \cdots \subsetneq P_m / (g)
        \]

        Thus \(\dim k[x_1 , \cdots , x_n] / (g) \geq m - 1\).

        Claim: \(\operatorname{tr deg Frac}(k[x_1 , \cdots , x_n] / (g)) = n-1\).

        By the induction hypothesis, \(n - 1 \geq m - 1\) so \(n \geq m\). We already know \(m \geq n\) so \(m = n\). So the claim implies the theorem.

        Proof of Claim: WLOG assume \(g\) is non-constant in \(x_n\).
        
        Then \(\{\overline{x}_1 , \cdots , \overline{x}_{n-1} \in k[x_1 , \cdots , x_n] / (g)\}\) is a transcendence basis of \(\operatorname{Frac} (k[x_1 , \cdots , x_n] / (g))\).

        Note that \(\overline{x}_n\) is algebraic over \(\overline{x}_1 , \cdots , \overline{x}_{n-1}\) by the equation \(g\). Then it is enough to show that \(\overline{x}_1 , \cdots , \overline{x}_{n-1}\) are algebraically independent.
        
        Say \(h(\overline{x}_1 , \cdots , \overline{x}_{n-1}) = 0\).
        
        Then \(h(x_1 , \cdots , x_{n-1}) \in (g)\). Then \(g \mid h\) which cannot happen since \(g\) is non-constant in \(x_n\).
    \end{proof}
    ...
    \begin{theorem}
        ...
        \[
            \dim X \times_{\operatorname{Spec} k} Y = \dim X + \dim Y
        \]
    \end{theorem}

    Example: note that \(\dim (\operatorname{Spec} \mathbb{Z}) = 1\).

    What about \(\operatorname{Spec} \mathbb{Z} \times_{\operatorname{Spec} \mathbb{Z}} \operatorname{Spec} \mathbb{Z}  = \mathbb{Z}\). This is a non-example.
    
    If \(A\) and \(B\) are f.g. \(k\)-algebras and also fintegral domains then,

    \[
        \dim A \otimes_k B = \dim A + \dim B
    \]

    One way to see this is Noether Normalization.

    \(A\) is module-finite over \(k[a_1, \cdots , a_n]\) and \(B\) is module-finite over \(k[b_1, \cdots , b_m]\) then \(A \otimes_k B\) is module finite over \(k[a_1 , \cdots , a_n] \otimes k[b_1 , \cdots , b_m] = k[a_1 \otimes 1, \cdots , a_n \otimes 1, 1 \otimes b_1 , \cdots , 1 \otimes b_m]\).

    \section*{Friday, 12/12/2025}
    

    \begin{theorem}
        Suppose we have fields \(K_1 \subset K_2 \subset K_3\).

        Then, \(\text{tr deg}_{K_1} K_3 = \text{tr deg}_{K_1} K_2 + \text{tr deg}_{K_2} K_3\)
    \end{theorem}

    \begin{proof}
        Suppose \(\{ \alpha_1 , \cdots , \alpha_m \} \in K_2\) is a transcendence basis over \(K_1\) and \(\{ \beta_1 , \cdots , \beta_n \in K_3 \}\) is a transcendence basis over \(K_2\).
        
        Then \(\alpha_1 , \cdots , \alpha_m, \beta_1 , \cdots , \beta_n\) is a transcendence basis of \(K_3\). 
        \[
            \begin{tikzcd}
                K_3 \ar[d, no head, "alg"]\\
                K_2(\beta_1 , \cdots , \beta_n) \ar[d, no head] \ar[dr, no head, "alg?"] \\
                K_2 \ar[d, no head, "alg"] & K_1(\alpha_1 , \cdots , \alpha_m , \beta_1 , \cdots , \beta_m) \ar[ld, no head] \\
                K_1(\alpha_1 , \cdots , \alpha_m) \ar[d, no head] \\
                K_1
            \end{tikzcd}
        \]
        ...
        \(P (\alpha_1 , \cdots , \alpha_m, \beta_1 , \cdots , \beta_n) = 0\)


        \(0 \neq P(x_1 , \cdots , y_n) \in K_1[x_1 , \cdots , y_n]\)

        \[
            \sum Q_{i_1 , \cdots , i_n}(x_1 , \cdots , x_m) y_1^{i_1} \cdots y_n^{i_n}
        \]

        Then \(0 = Q_{i_1 , \cdots , i_n}(\alpha_1 , \cdots , \alpha_m) \in K_2 \)
        

    \end{proof}

    \begin{theorem}
        Suppose \(X \to Y\) is a dominant morphism of \(k\)-varieties and \(\eta\) is generic. Then \(\dim X = \dim Y + \dim X_\eta\).
    \end{theorem}

    \begin{proof}
        Reduce to the case \(\operatorname{Spec} A \to \operatorname{Spec} B\) where \(A\) and \(B\) are integral domains.

        Let \(K_1 = k, K_2 = \mathbb{K}(\eta) = \operatorname{Frac} (B), K_3 = \operatorname{Frac}(A)\)
        
        \(B^c \xrightarrow{\pi^\sharp} A\) gives us \(K_1 \subset K_2 \subset K_3\). 
    \end{proof}

    \begin{theorem}
        If \(X \to \operatorname{Spec} k\) is of dimension \(n\) and \(K\) is a field extension of \(k\) then \(X \times_{\operatorname{Spec} k} \operatorname{Spec} K\) has dimension \(n\). 
    \end{theorem}

    \begin{theorem}
        [Cohen-Seidelberg; Going Down]

        Suppose \(B \subset A\) is an integral extension of integral domains and \(B\) is normal.

        If \(P_1 \subsetneq P_2 \subsetneq \cdots \subsetneq P_n\) in \(B\) and \(Q_n \in \operatorname{Spec} A\) maps to \(P_n = Q_n \cap B\) then \(\exists\) a chain of prime ideals \(Q_1 \subsetneq \cdots \subsetneq Q_n\) such that \(Q_i \mapsto P_i\). 
    \end{theorem}

    \begin{corollary}
        Given any maximal chain of prime ideals in \(A\):

        \[
            P_0 \subsetneq P_1 \subsetneq \cdots \subsetneq P_n
        \]

        in any finitely generated \(k\)-algebra \(A_1\) we have \(n = \dim A\).
    \end{corollary}
    ...
    \begin{proof}
        ...
        \begin{enumerate}[label=\arabic*)]
            \item Equivalent to proving the theorem for \(\mathbb{A}^n\). Indeed, use Noether normalization to map \(X \to \mathbb{A}^n\) which is integral and dominant. Use `going down' on this map.
            
            \(k[x_1 , \cdots , x_n] \subset A\). We can lift the `intermediate dimension' stuff on \(\mathbb{A}^n\) to \(A\).
            
            \item Suppose \(P_0 \subsetneq \cdots \subsetneq P_m\) is a maximal chain in \(k[x_1 , \cdots , x_n]\) but \(m < n\).
            
            Let \(f\in P_1\) be an irreducible polynomial so that \(P_0 \subsetneq (f) \subset P_1 \implies (f)=P_1\).

            \(k[x_1 , \cdots , x_n] / (f)\) has a maximal chain of prime ideals \(P_1 / P_1 \subsetneq P_2 / P_1 \subsetneq \cdots \subsetneq P_n / P_1\) with length \(m - 1\).

            Continuing, we see that we must have \(m = n\).
        \end{enumerate} 
    \end{proof}
    ...
    \begin{theorem}
        ...
        Let \(X\) be a noetherian scheme. Let \(f\in \Gamma(X,\mathcal{O}(X))\). Then every irreducible component of \(V(f)\) has codimension \(0\) or \(1\). 
    \end{theorem}

    \begin{theorem}
        If \(X \subset \mathbb{P}^n_k\) is any closed subset of \(\dim > 0\) and \(H\) is any hyperplane in \(\\PP\mathbb{P}^n_k\) then \(H\cap X \neq \varnothing\).
    \end{theorem}

    \begin{proof}
        Think about \(X\) corresponding to a homogeneous ideal \((P_m(x_0 , \cdots , x_n))\) in \(k[ x_0 , \cdots , x_n]\). This defines a cone.

        Then \(\dim \operatorname{cone}(X) = \dim X + 1\).
        
        Now consider hyperplane \(H\). Point \((0, \cdots , 0)\) is in the intersection when we project.

        \begin{figure}[H]
            \centering
            \includegraphics[width=0.8\textwidth]{img/cone}
        \end{figure}

        Take the cone of the intersection.
    \end{proof}

    Suppose \(X, Y\) are integral and \(X \to Y\) is dominant. Let \(\eta\in Y\) be generic point and \(X_\eta\) be the generic fiber.

    Then \(\dim X_\eta = \dim X - \dim Y\).

    Claim: \(\dim X_s = \dim X_\eta\) for all \(s\) in a dense open subset \(S\) of \(Y\).
    
    Think about the map \(\mathbb{A}^2 \to \mathbb{A}^2\) given by \((x,y) \mapsto (x,xy)\).

    Fiber of \((a,b)\) when \(a\neq 0\) is \((a,b / a)\) which is \(0\) dimensional.

    \((0,b) \mapsto (0,0)\) so fiber of \((0,0)\) is \(1\)-dimensional.

    So not all fiber dimensions have to be same, even though generic dimension is the same.

    Suppose \(y_1 , y_2 \in Y\) and \(y_2 \in \overline{\{ y_1 \} }\) and fibers \(X_{y_1}, X_{y_2}\) are non-empty.
    
    Then, \(\dim X_{y_2} \geq \dim X_{y_1}\).

    `The dimension of \(X_y\) is an upper semicontinuous function'.

    An integer-valued function \(f: X \to \mathbb{Z}\) is upper semicontinuous if \(f ^{-1} ((-\infty, a))\) is open for all \(z\in\mathbb{Z}\). 

    All non-empty fibers thus have \(\dim \geq \dim X - \dim Y\).

    In fact, every irreducible component of every non-empty fiber satisfies the same inequality.

    This describes how fibers of morphisms look like.
