...
\usepackage{amsmath, amsthm, amssymb, amsfonts, mathtools, enumitem, fancyhdr}
\usepackage{tikz-cd}
\usepackage{graphicx}
\usepackage{float}
\usepackage{booktabs}
...
\begin{definition}
    ...
    Its elements are the \underline{algebraic numbers}
...
This is not the same as the definition of a prime element. In general, irreduibles might not be equal to primes.
...
Theorem 2.1 \(\iff \mathcal{O}_K\) is a \underline{Dedekind domain}.
...
\begin{definition}
    Given a prime number \(p \in \mathbb{Z}_{> 0}\) we write \(p \cdot \mathcal{O}_K = P_1^{e_1} \cdots P_r^{e_r} \quad (\ast)\). \(p\) is called:
...
\begin{theorem}
    ...
    Then, the ideal \(P_i \coloneqq (p, \mu_i(\theta)) \in \operatorname{Max} (\mathcal{O})\) for \(1 \leq i \leq r\) and \(p\mathcal{O} =P_e^{e_1}\cdots P_r^{e_r}\)  
\end{theorem}
...
\begin{proof}
    ...
    If \(n = [K : \mathbb{Q}]\) then \(p^n = \underset{[\underset{\oplus_1^n \mathbb{Z} \alpha_i}{\mathcal{O}} : \underset{\oplus_1^n p \mathbb{Z} \alpha_i}{p \mathcal{O} ]}}N(p\mathcal{O}) \leq \prod_{i=1}^r (P_i^{e_i}) = \prod N(P_i)^{e_i} = \prod [\mathcal{O} : P_i]^{e_i}\). 
...
\begin{example}
    ...
    \(\overset{.}{=}\) means only finitely many exceptions.
...
\begin{proof}
    ...
    Then, \(S = \{ \mu(\gamma) \mid \gamma \in \Gamma \} \subseteq \Gamma \cap \overline{\Phi}_0\). 
    ...
    Thus, \(\Gamma\) is contained in a finitely generated abelian group of rank \(m\), therefore \(\Gamma\) must be a finitely generated abelian group of rank \(r \leq m\). 

    Thus \(\Gamma\) must be a lattice.
...
\begin{example}
    ...
    \[
        \underset{a\cdot [3,\omega]}{\operatorname{Cl}_K} \underset{\leftarrow}{\cong} \underset{a + 4\mathbb{Z}}{\mathbb{Z} / 4\mathbb{Z}}  
    \]
...
Suppose \(\mu_K\) is the group of roots of unity in \(K\). \(\mu_K\) must be finite.
